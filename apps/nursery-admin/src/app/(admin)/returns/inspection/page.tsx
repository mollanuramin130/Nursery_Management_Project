...
const QUEUE = ["RECEIVED", "UNDER_INSPECTION"];
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description="Inspect received items, set disposition, and optionally create a refund."
      />
      ...
      {!loading && !error ? (
        rows.length === 0 ? (
          <EmptyState title="No returns awaiting inspection" />
        ) : (
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--admin-border)]/70">
                    ...
                    <td className="px-3 py-2.5">
                      <Badge tone="warning">{row.status.replaceAll("_", " ")}</Badge>
                    </td>
