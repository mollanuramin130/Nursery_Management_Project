...
Route::post('plant-finder/match', [PlantFinderController::class, 'match'])->middleware('throttle:plant-finder');
...
Route::get('search/suggestions', SearchSuggestionController::class)->middleware('throttle:search');
Route::get('search/assist', SearchAssistController::class)->middleware('throttle:search');

Route::post('product-views', [ProductViewController::class, 'store'])
    ->middleware(['optional.jwt', 'throttle:product-views']);
Route::get('recently-viewed', [ProductViewController::class, 'recent'])
    ->middleware(['optional.jwt', 'throttle:recently-viewed']);
...
Route::middleware(['auth:api', 'active.user'])->group(function () {
    ...
    Route::post('products/{id}/stock-alert', [StockAlertController::class, 'store'])->whereNumber('id')->middleware('throttle:stock-alert');
