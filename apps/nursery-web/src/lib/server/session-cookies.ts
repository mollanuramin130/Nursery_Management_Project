/**
 * QA-33 — HttpOnly session cookies for Customer Web BFF.
 * Tokens never leave the server into browser JS.
 */
import { cookies } from "next/headers";
import { NextResponse } from "next/server";
import { resolveCookieSecureFlag } from "@/lib/session-cookie-policy";

export const ACCESS_COOKIE = "gl_web_access";
export const REFRESH_COOKIE = "gl_web_refresh";
export const CSRF_COOKIE = "gl_web_csrf";
export const CSRF_HEADER = "x-csrf-token";

/** Legacy localStorage keys — cleared client-side on bootstrap. */
export const LEGACY_ACCESS_KEY = "gl_access_token";
export const LEGACY_REFRESH_KEY = "gl_refresh_token";

const ACCESS_MAX_AGE = 60 * 60; // match typical JWT_TTL minutes→seconds (60m)
...
export function refreshCookieOptions(maxAge = REFRESH_MAX_AGE) {
  return {
    ...
    secure: cookieSecure(),
    sameSite: "lax" as const,
    // Narrow to BFF only (auth + proxy server-side refresh). Never Path=/.
    path: "/api/bff",
    maxAge,
  };
}
...
/** Mutating BFF calls require CSRF header matching the readable cookie. */
